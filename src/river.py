...
def defense(seat, card_rank, equity, pot, odd, valid_actions, vi_action, action_line, behind):
    if seat == 1:
        if vi_action["action"] == "RAISE":
            ...
            if pot == 20 and equity >= 0.5 and odd <= 0.34:
                action = "call"
                amount = valid_actions[1]["amount"]
            else:
                if card_rank >= 2 and equity >= 0.6:
                    action = "call"
                    amount = valid_actions[1]["amount"]
        ...
        if not vi_action:
            ...
            elif card_rank >= 2 and equity >= 0.65 and odd <= 0.34:
                action = "call"
                amount = valid_actions[1]["amount"]
...
def attack(seat, card_rank, equity, pot, odd, valid_actions, vi_action, action_line, behind):
    if seat == 1:
        if vi_action["action"] == "RAISE":
            action_line.vi_status = 'a'
            if card_rank >= 2 and equity >= 0.7:
                action = "call"
                amount = valid_actions[1]["amount"]
            elif card_rank > 0 and equity >= 0.7 and odd <= 0.28:
                action = "call"
                amount = valid_actions[1]["amount"]
            else:
                amount = 0
            ...
            action_line.vi_status = 'p'
            if card_rank > 0 and equity >= 0.65:
                action = "raise"
                amount = max(bet(pot, 100, 100), valid_actions[2]["amount"]["min"])
            else:
                action = "call"
...
def river_strategy(seat, hole_card, card_info, card_rank, valid_actions, round_state, action_line, equity, behind):
    ...
    if len(history) == 0:
        ...
        vi_action = history[-1]
    if action_line.my_his["turn"][-1][0] == "CALL":
