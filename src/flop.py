...
def defense(seat, card_rank, equity, pot, odd, valid_actions, vi_action, action_line, behind):
    if seat == 1:
        ...
        if not vi_action:
            ...
            elif card_rank >= 2 and equity >= 0.55 and odd <= 0.34:
                action = "call"
                amount = valid_actions[1]["amount"]
            elif card_rank > 0 and equity >= 0.55 and odd <= 0.28:
                action = "call"
                amount = valid_actions[1]["amount"]
            else:
                amount = 0 
...
def attack(seat, card_rank, equity, pot, odd, valid_actions, vi_action, action_line, behind):
    if seat == 1:
        ...
        if not vi_action:
            ...
            if card_rank >= 2 and equity >= 0.70:
                action = "call"
                amount = valid_actions[1]["amount"]
            else:
                action = "fold"
...
def flop_strategy(seat, hole_card, card_info, card_rank, valid_actions, round_state, action_line, equity, behind):
    ...
    if len(history) == 0:
        ...
        vi_action = history[-1]
    if action_line.my_his["preflop"][-1][0] == "CALL":
        action_line.status = 'p'
        if self.pot_type == "limp":
            action_line.vi_status = 'p'
        else:
            action_line.vi_status = 'a'
    else:
